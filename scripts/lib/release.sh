...
# PIGPIG github account info
readonly PIGPIG_GITHUB_ORG=marmotedu
readonly PIGPIG_GITHUB_REPO=iam

readonly ARTIFACT=iam.tar.gz
readonly CHECKSUM=${ARTIFACT}.sha1sum

PIGPIG_BUILD_CONFORMANCE=${PIGPIG_BUILD_CONFORMANCE:-y}
PIGPIG_BUILD_PULL_LATEST_IMAGES=${PIGPIG_BUILD_PULL_LATEST_IMAGES:-y}
...
function iam::release::updload_tarballs() {
  ...
  do
    if [ "${COSTOOL}" == "coscli" ];then
      coscli cp "${file}" "cos://${BUCKET}/${COS_RELEASE_DIR}/${PIGPIG_GIT_VERSION}/"
      coscli cp "${file}" "cos://${BUCKET}/${COS_RELEASE_DIR}/latest/"
    else
      coscmd upload  "${file}" "${COS_RELEASE_DIR}/${PIGPIG_GIT_VERSION}/"
...
function iam::release::package_src_tarball() {
  ...
  if [[ "${PIGPIG_GIT_TREE_STATE-}" = 'clean' ]]; then
    git archive -o "${src_tarball}" HEAD
  else
    find "${PIGPIG_ROOT}" -mindepth 1 -maxdepth 1 \
      ! \( \
      \( -path "${PIGPIG_ROOT}"/_\* -o \
      -path "${PIGPIG_ROOT}"/.git\* -o \
      -path "${PIGPIG_ROOT}"/.gitignore\* -o \
      -path "${PIGPIG_ROOT}"/.gsemver.yaml\* -o \
      -path "${PIGPIG_ROOT}"/.config\* -o \
      -path "${PIGPIG_ROOT}"/.chglog\* -o \
      -path "${PIGPIG_ROOT}"/.gitlint -o \
      -path "${PIGPIG_ROOT}"/.golangci.yaml -o \
      -path "${PIGPIG_ROOT}"/.goreleaser.yml -o \
      -path "${PIGPIG_ROOT}"/.note.md -o \
      -path "${PIGPIG_ROOT}"/.todo.md \
      \) -prune \
      \) -print0 \
      | "${TAR}" czf "${src_tarball}" --transform "s|${PIGPIG_ROOT#/*}|iam|" --null -T -
  fi
}
...
function iam::release::package_server_tarballs() {
  ...
  if [[ -n ${PIGPIG_BUILD_PLATFORMS-} ]]; then
    read -ra long_platforms <<< "${PIGPIG_BUILD_PLATFORMS}"
  fi
  ...
  for platform_long in "${long_platforms[@]}"; do
    ...
    local server_bins=("${PIGPIG_SERVER_BINARIES[@]}")
...
function iam::release::package_client_tarballs() {
  ...
  for platform_long in "${long_platforms[@]}"; do
    ...
    local client_bins=("${PIGPIG_CLIENT_BINARIES[@]}")
...
function iam::release::build_server_images() {
  ...
  local platform
  for platform in "${PIGPIG_SERVER_PLATFORMS[@]}"; do
    local platform_tag
    local arch
    platform_tag=${platform/\//-} # Replace a "/" for a "-"
    ...
    # This fancy expression will expand to prepend a path
    # (${LOCAL_OUTPUT_BINPATH}/${platform}/) to every item in the
    # PIGPIG_SERVER_IMAGE_BINARIES array.
    cp "${PIGPIG_SERVER_IMAGE_BINARIES[@]/#/${LOCAL_OUTPUT_BINPATH}/${platform}/}" \
      "${release_stage}/server/bin/"
...
function iam::release::create_docker_images_for_server() {
  ...
  (
    ...
    # We can use PIGPIG_DOCKER_REGISTRY to include and extra registry in the docker archive.
    # If we use PIGPIG_DOCKER_REGISTRY="k8s.gcr.io", then the extra tag (same) is ignored, see release_docker_image_tag below.
    local -r docker_registry="k8s.gcr.io"
    # Docker tags cannot contain '+'
    local docker_tag="${PIGPIG_GIT_VERSION/+/_}"
    if [[ -z "${docker_tag}" ]]; then
      iam::log::error "git version information missing; cannot create Docker tag"
      return 1
    fi

    # provide `--pull` argument to `docker build` if `PIGPIG_BUILD_PULL_LATEST_IMAGES`
    # is set to y or Y; otherwise try to build the image without forcefully
    # pulling the latest base image.
    local docker_build_opts
    docker_build_opts=
    if [[ "${PIGPIG_BUILD_PULL_LATEST_IMAGES}" =~ [yY] ]]; then
    ...
    for wrappable in $binaries; do
      ...
      (
        ...
        ln "${PIGPIG_ROOT}/build/nsswitch.conf" "${docker_build_path}/nsswitch.conf"
...
EOF
        ...
        local -r release_docker_image_tag="${PIGPIG_DOCKER_REGISTRY-$docker_registry}/${binary_name}-${arch}:${PIGPIG_DOCKER_IMAGE_TAG-$docker_tag}"
    ...
    if [[ "${PIGPIG_BUILD_CONFORMANCE}" =~ [yY] ]]; then
...
function iam::release::package_iam_manifests_tarball() {
  ...
  local src_dir="${PIGPIG_ROOT}/deployments"
  ...
  #cp "${PIGPIG_ROOT}/cluster/gce/gci/health-monitor.sh" "${dst_dir}/health-monitor.sh"
...
Client binaries are no longer included in the PIGPIG final tarball.
...
EOF
  ...
  cp -R "${PIGPIG_ROOT}/scripts/release" "${release_stage}/"
...
os=linux arch=amd64 version=${PIGPIG_GIT_VERSION} && wget https://${BUCKET}.cos.${REGION}.myqcloud.com/${COS_RELEASE_DIR}/\$version/{iam-client-\$os-\$arch.tar.gz,iam-server-\$os-\$arch.tar.gz}
EOF
...
Server binary tarballs are no longer included in the PIGPIG final tarball.
...
EOF
  ...
  #cp -R "${PIGPIG_ROOT}/hack/lib" "${release_stage}/hack/"

  cp -R ${PIGPIG_ROOT}/{docs,configs,scripts,deployments,init,README.md,LICENSE} "${release_stage}/"

  echo "${PIGPIG_GIT_VERSION}" > "${release_stage}/version"
...
function iam::release::verify_prereqs(){
  ...
  if [ -z "$(which ${COSTOOL} 2>/dev/null)" ]; then
    ...
    if ! make -C "${PIGPIG_ROOT}" tools.install.${COSTOOL}; then
...
function iam::release::github_release() {
  ...
  iam::log::info "create a new github release with tag ${PIGPIG_GIT_VERSION}"
  github-release release \
    --user ${PIGPIG_GITHUB_ORG} \
    --repo ${PIGPIG_GITHUB_REPO} \
    --tag ${PIGPIG_GIT_VERSION} \
    --description "" \
    --pre-release

  # update iam tarballs
  iam::log::info "upload ${ARTIFACT} to release ${PIGPIG_GIT_VERSION}"
  ...
  iam::log::info "upload iam-src.tar.gz to release ${PIGPIG_GIT_VERSION}"
  github-release upload \
    --user ${PIGPIG_GITHUB_ORG} \
    --repo ${PIGPIG_GITHUB_REPO} \
    --tag ${PIGPIG_GIT_VERSION} \
...
function iam::release::generate_changelog() {
  iam::log::info "generate CHANGELOG-${PIGPIG_GIT_VERSION#v}.md and commit it"

  git-chglog ${PIGPIG_GIT_VERSION} > ${PIGPIG_ROOT}/CHANGELOG/CHANGELOG-${PIGPIG_GIT_VERSION#v}.md

  set +o errexit
  git add ${PIGPIG_ROOT}/CHANGELOG/CHANGELOG-${PIGPIG_GIT_VERSION#v}.md
  git commit -a -m "docs(changelog): add CHANGELOG-${PIGPIG_GIT_VERSION#v}.md"
