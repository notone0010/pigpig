...
iam::util::ensure-temp-dir() {
  if [[ -z ${PIGPIG_TEMP-} ]]; then
    PIGPIG_TEMP=$(mktemp -d 2>/dev/null || mktemp -d -t iamrnetes.XXXXXX)
...
# $PIGPIG_ROOT must be set
iam::util::find-binary-for-platform() {
  local -r lookfor="$1"
  local -r platform="$2"
  local locations=(
    "${PIGPIG_ROOT}/_output/bin/${lookfor}"
    "${PIGPIG_ROOT}/_output/${platform}/${lookfor}"
    "${PIGPIG_ROOT}/_output/local/bin/${platform}/${lookfor}"
    "${PIGPIG_ROOT}/_output/platforms/${platform}/${lookfor}"
  )
...
# Removes previously generated docs-- we don't want to check them in. $PIGPIG_ROOT
# must be set.
iam::util::remove-gen-docs() {
  if [ -e "${PIGPIG_ROOT}/docs/.generated_docs" ]; then
    # remove all of the old docs; we don't want to check them in.
    while read -r file; do
      rm "${PIGPIG_ROOT}/${file}" 2>/dev/null || true
    done <"${PIGPIG_ROOT}/docs/.generated_docs"
