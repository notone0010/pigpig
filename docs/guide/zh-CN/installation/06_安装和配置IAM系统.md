...
$ sudo mkdir -p ${IAM_DATA_DIR}/{pigpig,iam-authz-server,iam-pump,iam-watcher} # 创建 Systemd WorkingDirectory 目录
...
$ sudo mv pigpig*pem ${IAM_CONFIG_DIR}/cert # 将生成的证书和私钥文件拷贝到配置文件目录
```
...
$ systemctl status pigpig # 查看 pigpig 运行状态，如果输出中包含 active (running)字样说明 pigpig 成功启动。
```
