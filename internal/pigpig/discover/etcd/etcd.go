...
func (ds *datastore) Register() discover.ServiceRegister {
	return newRegister(ds)
}

func (ds *datastore) Discover() discover.ServiceDiscover {
	return newDiscover(ds)
}

func (ds *datastore) Info() discover.InfoStore {
	return newInfo(ds)
}
...
var (
	etcdFactory discover.Factory
	once        sync.Once
)

// GetEtcdFactoryOr create a etcdFactory discover with given options.
func GetEtcdFactoryOr(opt *genericoptions.EtcdOptions, onKeepaliveFailure func()) (discover.Factory, error) {
	if opt == nil && etcdFactory == nil {
		return nil, fmt.Errorf("failed to get etcd discover fatory")
	}
	...
	if etcdFactory == nil || err != nil {
		return nil, fmt.Errorf("failed to get etcd discover fatory, etcdFactory: %+v, error: %w", etcdFactory, err)
	}
